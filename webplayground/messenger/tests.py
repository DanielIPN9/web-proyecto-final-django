...
class ThreadTestCase(TestCase):
    ...
    # Validacino de la creacion de los hilos
    def test_add_users_to_thread(self):
        self.thread.users.add(self.user1, self.user2)
        self.assertEqual(len(self.thread.users.all()), 2)
    # Asignacion de usuarios a un hilo
    def test_filter_thread_by_users(self):
        self.thread.users.add(self.user1, self.user2)
        threads = Thread.objects.filter(users=self.user1).filter(users=self.user2)
        self.assertEqual(self.thread, threads[0])
    # Recuperacion de hilos a partir de los usuarios
    def test_filter_non_existent_thread(self):
        threads = Thread.objects.filter(users=self.user1).filter(users=self.user2)
        self.assertEqual(len(threads), 0)
    # Comprobacion del envio de los mensajes entre dos usuarios
    def test_add_messages_to_thread(self):
        ...
        for mesasage in self.thread.messages.all():
            print("({}): {}".format(mesasage.user, mesasage.content))
    # debug para comprobar mensaje intruso 
